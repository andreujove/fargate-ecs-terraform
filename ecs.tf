  ...
  container_definitions = jsonencode([
    {
      ...
      image     = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/tp-flask-app:1.0.0"
      essential = true
...
resource "aws_ecs_service" "main" {
  ...
  network_configuration {
    ...
    subnets          = aws_subnet.private[*].id # Modern splat syntax
    assign_public_ip = false
  }

  load_balancer {
    target_group_arn = aws_lb_target_group.ecs_tg.arn # Use .arn instead of .id
